-- Script que balanceia TANTO português quanto inglês simultaneamente
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- If we need to change, swap the TEXT content in BOTH languages
      IF should_change THEN
        -- Extract the TEXT of the current correct answer (PORTUGUÊS)
        CASE question_record.correct_answer
        ...
        -- Extract the TEXT of the current correct answer (INGLÊS)
        CASE question_record.correct_answer
          WHEN 'a' THEN correct_answer_text_en := question_record.options_en->0->>'text';
          WHEN 'b' THEN correct_answer_text_en := question_record.options_en->1->>'text';
          WHEN 'c' THEN correct_answer_text_en := question_record.options_en->2->>'text';
          WHEN 'd' THEN correct_answer_text_en := question_record.options_en->3->>'text';
        END CASE;
        
        -- Extract all option texts (PORTUGUÊS)
        option_texts := ARRAY[
        ...
        -- Extract all option texts (INGLÊS)
        ...
        -- Swap the correct answer text to the target position (BOTH LANGUAGES)
        CASE target_position
          WHEN 'a' THEN
            ...
            -- Put the displaced texts in the original position
            CASE question_record.correct_answer
              WHEN 'b' THEN 
        ...
        -- Build new options with FIXED IDs but swapped texts (PORTUGUÊS)
        ...
        -- Build new options with FIXED IDs but swapped texts (INGLÊS)
        ...
        -- Update the question with BOTH languages synchronized
        ...
        RAISE NOTICE 'Question %: Moved correct answer text from position % to position % (BOTH PT and EN)', 
  ...
  RAISE NOTICE 'Rebalancing completed. Option IDs remain fixed, TEXT content was swapped in BOTH Portuguese and English.';
END;
$$;