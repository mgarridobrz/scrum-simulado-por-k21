...
-- Batch update: Correctly escaped English translations for questions 1–40.
...
-- (Continue with questions 11 through 40 as provided, making sure all string escapes are handled, especially for apostrophes in strings like Owner's, team can't, etc.)

-- Example for a question with apostrophe:
UPDATE quiz_questions SET 
    question_en = 'What does it mean to say the Product Owner is ''one person, not a committee''?',
    explanation_en = 'It means that a single person must have the final authority and accountability for Product Backlog decisions. While the Product Owner may represent the interests of many stakeholders and can gather input, ultimately only one person owns the Product Backlog decisions.',
    options_en = '[
        {"id":"a","text":"The Product Owner never consults others"},
...
-- Repeat this pattern for every question up to 40 in your JSON, always using two single quotes to escape any literal apostrophes (e.g., Owner''s, team''s, can''t, etc.)
